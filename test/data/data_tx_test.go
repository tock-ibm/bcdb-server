...
func TestAsyncDataTx(t *testing.T) {
	...
	t.Run("multiple blind writes to the same key on the same block", func(t *testing.T) {
		...
		allIn := func() bool {
			for _, txEnv := range txEnvs {
				...
				count := 0
				for _, v := range resp.GetResponse().GetReceipt().GetHeader().GetValidationInfo() {
					if v.Flag == types.Flag_VALID {
						count++
					}
				}
				// only the first 5 key writes in the block are valid
	...
	// TODO check whether the following synch tests can be made asynch as well, in the style from above
